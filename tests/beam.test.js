...
test('консоль под равномерной нагрузкой (балка с вылетом)', () => {
  ...
  const iL = Math.round((L / (L + a)) * (r.x.length - 1));
  close(r.M[iL], -(w * a * a) / 2, 5e-3, 'опорный момент −wa²/2');
});
...
test('двухпролётная неразрезная балка', () => {
  ...
  const iM = Math.round((r.x.length - 1) / 2);
  close(r.M[iM], -(w * L * L) / 8, 1e-2, 'момент над средней опорой −wL²/8');
