...
function frostWarnings(res) {
  ...
  return [`<div class="warn bad"><b>Блок выдавит морозом: грунт тянет вверх ${kN(h.pull)} кН, держат ${kN(h.F)} кН.</b>
    ...
    не считает. Трение о талый грунт ниже промерзания учтено по табл. 7.3 СП 24 — ${kN(h.Frf)} кН. ${more}.</div>`];
}
  ...
  $('report').innerHTML = `
    ...
    <table>
      ...
      <tr><td>Касательные силы пучения</td><td>${(() => {
        const h = res.bases.outer.heave;
        if (h.applies) return `τ_fh·A_fh = ${f2(h.pull / 1000)} кН против F = ${f2(h.F / 1000)} кН (постоянная нагрузка и блок при γ_f = 0,9) плюс трение о талый грунт ${f2(h.Frf / 1000)} кН / 1,1 — ${h.check.U > 1 ? 'не проходит' : 'проходит'} (СП 22, ф. (6.35), (6.38), табл. 6.12; СП 24, табл. 7.3)`;
