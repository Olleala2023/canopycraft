...
function frostWarnings(res) {
  ...
  return [`<div class="warn bad"><b>Блок выдавит морозом: грунт тянет вверх ${kN(h.pull)} кН, держат ${kN(h.F)} кН.</b>
    ...
    не считает. Трение о талый грунт ниже промерзания пока не учтено — в запас. ${more}.</div>`];
}
  ...
  $('report').innerHTML = `
    ...
    <table>
      ...
      <tr><td>Касательные силы пучения</td><td>${(() => {
        const h = res.bases.outer.heave;
        if (h.applies) return `τ_fh·A_fh = ${f2(h.pull / 1000)} кН против F = ${f2(h.F / 1000)} кН (постоянная нагрузка и блок при γ_f = 0,9) — ${h.check.U > 1 ? 'не проходит' : 'проходит'}; трение о талый грунт не учтено (СП 22, ф. (6.35), табл. 6.12)`;
    ...
    ${spliceReportNote(b)}
    <h2>8. Ограничения</h2>
    <p>Расчёт не охватывает: расчёт основания по грунту (несущая способность и осадка), трение о талый
    грунт ниже промерзания в проверке на выпучивание (в запас), анкерные уширения и другие конструктивные
    меры против пучения, диагонали в плоскости кровли, огнестойкость, температурные воздействия. Опирание стропил принято шарнирным. Внецентренное сжатие столбов проверено
