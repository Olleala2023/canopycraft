...
/**
 * Вырыв конуса у края блока: проекция конуса под 45° — круг радиусом h_ef,
 * и часть его, которая оказалась за гранью бетона, не работает. Та же модель,
 * что у anchorCone, только площадь честная: круг минус сегмент за гранью.
 */
export function anchorConeEdge(N, hef, c, Rbt, note) {
  const r = hef;
  const cut = c >= r ? 0 : r * r * Math.acos(Math.max(-1, c / r)) - c * Math.sqrt(Math.max(0, r * r - c * c));
  const area = Math.PI * r * r - cut;
  return chk('Вырыв конуса бетона у края блока', Math.abs(N), Rbt * area, 'Н',
    'N ≤ R_bt·A, A — круг радиусом h_ef без сегмента за гранью блока', note);
}

/**
 * Смятие бетона у края блока под плитой-«столиком»: столб стоит у края плиты,
 * бетон начинается за зазором. Прямоугольная эпюра напряжений R_b шириной Y у
 * края блока и растяжение дальнего ряда анкеров уравновешивают сжатие столба;
 * решение есть, только пока σ_тр ≤ R_b — иначе бетону у края не хватает места.
 */
export function edgeBearing(sigmaNeed, Rb, note) {
  return chk('Смятие бетона у края блока', sigmaNeed, Rb, 'МПа',
    'σ_тр = 2·N·(u_о − u_ст) / (B·d²) ≤ R_b, d = u_о − зазор', note);
}

/** Изгиб плиты-«столика» у грани столба: M — от анкеров и отпора бетона за гранью. */
export function sidePlateBending(sigma, Ry, note) {
  return chk('Изгиб плиты-столика', sigma, Ry, 'МПа', 'σ = 6·M/(B·t²) ≤ R_y', note);
}
