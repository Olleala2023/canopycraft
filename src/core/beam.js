...
export function solveBeam(o) {
  ...
  // --- эпюры статикой
  // Сетка вывода — равномерная плюс точки, где у эпюр излом или скачок: опоры,
  // силы, моменты, стыки. На одной равномерной сетке они проскакивают между
  // точками, и пик момента над опорой занижается на V·dx/2 (у стропила по
  // умолчанию — на 2 %). Перед каждой такой точкой ставится ещё одна вплотную
  // слева: там Q до скачка, который даёт реакция или сила.
  const JUMP = 1e-6; // мм
  const grid = new Set();
  for (let i = 0; i <= 400; i++) grid.add((length * i) / 400);
  const kinks = [...o.supports, ...point.map((p) => p.x), ...moments.map((p) => p.x), ...(o.hinges ?? [])];
  for (const k of kinks) {
    if (k <= 0 || k > length) continue;
    grid.add(k);
    grid.add(k - JUMP);
  }
  const xs = Float64Array.from([...grid].sort((a, b) => a - b).filter((v, i, a) => i === 0 || v - a[i - 1] > JUMP / 2));
  const nSample = xs.length;
  ...
  for (let i = 0; i < nSample; i++) {
    const x = xs[i];
    if (i > 0) {
      const xa = xs[i - 1], dx = x - xa, xm = xa + dx / 2;
