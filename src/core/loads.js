...
export function snowDrift(o) {
  ...
  // как только упёрся — снег, который не помещается по высоте, растекается
  // дальше, и b считается по формуле (Б.6), но не более 5h и не более 16 м.
  // Текст (Б.6) распознан неразборчиво, поэтому берётся её верхняя граница 5h —
  // это в запас: чем длиннее зона, тем больше снега на нижнем покрытии.
  const spread = raw > capGeom;
  const length = Math.min(16, spread ? 5 * h : 2 * h) * 1000;
  const governs =
  ...
  return { mu, raw, capGeom, capAbs, governs, length, spread, h, l1, l2, m1, m2 };
}
