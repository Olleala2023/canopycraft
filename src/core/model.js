...
export function defaultModel() {
  ...
  return {
    ...
    site: {
      ...
      soil: 'clay',
      /**
       * Состояние пучинистого грунта — строка табл. 6.12 СП 22 для касательных
       * сил пучения. По умолчанию первая строка, самая тяжёлая: по ней же
       * берётся обратная засыпка котлована.
       */
      soilState: 'wet',
      /** Геотехническая категория 1 — τ_fh × 0,9 (прим. 5 к табл. 6.12). */
      geoCat1: false,
    },
    roofing: 'prof8',
    rafters: {
     ...
     * μ = 1; 'roof' — две диагонали в плоскости кровли от прогона к обвязке
     * в крайней ячейке шириной roofBays шагов стропил, μ = 1. По умолчанию
     * ничего, как и раньше: программа не вправе считать, что связь есть.
     */
    bracing: { along: 'none', bays: 1, roofBays: 2, sectionId: 's40x40x3' },
     ...
     */
    postBase: {
      id: 'plate4m12', footing: 500, depth: 1200,
      /** Поверхность блока — коэффициент к τ_fh: залит в яму, неровности до 20 мм. */
      surface: 'rough20',
      /**
       * Мера против выпучивания: 'none' — нет; 'replace' — пазухи засыпаны
       * непучинистым грунтом (п. 6.8.12 СП 22), грунт у боковой поверхности
       * блока не пучится, и проверка касательных сил не применяется.
       */
      antiHeave: 'none',
    },
    ...
    wallPosts: {
      ...
      wallThickness: 300,
      /**
       * Фундамент столба у стены: 'beside' — отдельный блок рядом с фундаментом
       * дома, за прокладкой, столб на плите-«столике» у края блока, шпильки в
       * стену на овальных отверстиях; 'axis' — блок по оси столба, как у
       * наружных (прежняя схема: блок наполовину уходит под стену).
       */
      footing: 'beside',
    },
    /**
     * Стена дома — для ориентира и для проверки, не встал ли столб на проём
     * (core/house.js). width 0 — ширина не задана, стена по ширине навеса;
     * offset — от левого угла дома до левого края навеса. Окна и двери
     * { kind: 'window' | 'door', x, w, h, bottom }, мм: x от левого угла дома,
     * bottom — низ проёма от земли. Своих проёмов программа не придумывает.
     */
    house: { width: 0, offset: 0, openings: [] },
...
/**
 * Блок столба у стены стоит рядом с фундаментом дома, а не под столбом:
 * блок по оси столба наполовину уходил бы под стену, в ленту дома.
 */
export const wallBeside = (model) => (model.wallPosts.footing ?? 'beside') === 'beside';
